...
public class ArduinoController : MonoBehaviour
{
    ...
    [ValueDropdown("ports"), InlineButton("UpdatePorts")]
    public string portName = "COM3";

    [Tooltip("Baud rate that the serial device is using to transmit data.")]
    [ValueDropdown("BaudRates")]
    public int baudRate = 9600;
    private static int[] BaudRates = new int[] {2400, 4800, 9600, 19200, 31250, 38400, 57600, 74880, 115200};
    ...
    void Update()
    {
        ...
        if (ReferenceEquals(message, SERIAL_DEVICE_CONNECTED))
            SendMessage("OnConnectionEvent", true, SendMessageOptions.DontRequireReceiver);
        else if (ReferenceEquals(message, SERIAL_DEVICE_DISCONNECTED))
            SendMessage("OnConnectionEvent", false, SendMessageOptions.DontRequireReceiver);
        else
            SendMessage("OnMessageArrived", message, SendMessageOptions.DontRequireReceiver);
    }
    ...
    void UpdatePorts()
    {
        ...
        //Mac
         ...
         if (p == 4 || p == 128 || p == 6) {
             ...
             //foreach(string s in ttys)
                //Debug.Log(s);

            
             for (int i = ttys.Length - 1; i >= 0 ; i--)
             {
                 if (ttys[i].StartsWith ("/dev/cu"+"."))
                 {
                     string dev = ttys[i].Substring(("/dev/cu"+".").Length);
